...
def linux_build(cpu_type):
    ...
    with switch_dir("third_party"):
        ...
        if not os.path.exists("buildroot"):
            ret = os.system(f"git clone --depth 1 --single-branch -b {buildroot_version} {buildroot_url}")
            if ret != 0:
                return ret
...
""")
    ...
    # -----------------------------------
    with open(soc_json) as json_file:
        json_content = json.load(json_file)
    ...
    if args.generate_dtb:
        generate_dts(soc_json, rootfs=args.rootfs, cpu_type=cpu_type)
        if compile_dts(soc_json) != 0:
            return ErrorCode.DTS_ERROR
        if combine_dtb(soc_json) != 0:
            return ErrorCode.DTS_ERROR
        if copy_dtb(soc_json) != 0:
            return ErrorCode.DTS_ERROR
    ...
    # ------------
    if args.copy_images:
        if linux_copy_images(soc_json=soc_json) != 0:
            return ErrorCode.COPY_ERROR

    # TFTP-Prepare.
    # -------------
    if args.prepare_tftp:
        if linux_prepare_tftp(soc_json=soc_json) != 0:
